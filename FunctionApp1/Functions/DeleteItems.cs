...
namespace CRUD
{
    public class DeleteItems
    {
        ...
        public DeleteItems(ILoggerFactory loggerFactory, SourceCosmosClient srcContainer)
        {
        ...
        public async Task<HttpResponseData> RunAsync([HttpTrigger(AuthorizationLevel.Function, "get", "post")] HttpRequestData req)
        {
            ...
            while (queryIterator.HasMoreResults)
            {
                ...
                foreach (ToDoItem item in queryResult)
                {
                    try
                    {
                        var documentId = Convert.ToString(item.id);
                        ItemResponse<ToDoItem> responseStatus = await _srcClient._container.DeleteItemAsync<ToDoItem>(documentId, new PartitionKey(documentId));
                        ...
                        {
                            Console.WriteLine($"Item deletion failed with status code: {responseStatus.StatusCode}");
                        }
                    }
            ...
            response.WriteString($"{counts} items deleted in {timeElapsed}");
            return response;
        }
    }
}
